...
use crate::page::Page;
use crate::page_layout::{FromByte, INTERNAL_NODE_HEADER_SIZE, INTERNAL_NODE_NUM_CHILDREN_OFFSET, IS_ROOT_OFFSET, KEY_SIZE, LEAF_NODE_HEADER_SIZE, LEAF_NODE_NUM_PAIRS_OFFSET, NODE_TYPE_OFFSET, PARENT_POINTER_OFFSET, PTR_SIZE, VALUE_SIZE};

// Node表示b树中被内存中的单个页面占用的节点
#[derive(Clone, Debug)]
pub struct Node {
...
impl Node {
    ...
    /// split通过将给定节点围绕中位数分成两部分来创建一个兄弟节点。
    /// split将在b处拆分子节点，留下[0,b-1]键
    ///同时将[b, 2b-1]键集移动到兄弟节点。
